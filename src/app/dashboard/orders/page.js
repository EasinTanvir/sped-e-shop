...
export default async function DashboardOrdersPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <div>
        <p className="text-sm font-black uppercase tracking-wide text-red-600">Customer orders</p>
        <h1 className="mt-2 text-4xl font-black">Order management</h1>
        <p className="mt-2 text-zinc-600">View COD order details and send email notifications when status changes.</p>
      </div>
      ...
      <section className="mt-8 overflow-hidden rounded-md border border-zinc-200 bg-white shadow-sm">
        ...
        {orders.length ? orders.map((order) => (
          <div key={order.id} className="grid gap-3 border-b border-zinc-100 p-3 text-sm last:border-b-0 lg:grid-cols-[1fr_1fr_1.2fr_0.8fr_1.4fr_0.5fr]">
            <span>
            ...
            <Link href={`/dashboard/orders/${order.id}`} className="font-black capitalize text-red-600">
