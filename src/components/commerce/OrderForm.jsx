...
export default function OrderForm({ product, user }) {
    ...
    return (
      <div className="rounded-md border border-zinc-200 bg-white p-6 shadow-sm">
        ...
        <h3 className="mt-2 text-2xl font-black text-zinc-950">
          Sign in to place a COD order
        </h3>
        <p className="mt-2 text-sm leading-6 text-zinc-600">
          Customers must have an account before ordering. After login, your
          order will be saved in your customer panel.
        </p>
  ...
  return (
    <form
      ...
      className="space-y-4 rounded-md border border-zinc-200 bg-white p-5 shadow-sm"
    >
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-600">
          Signed in as{" "}
          <span className="font-black text-zinc-950">{user.email}</span>. Pay{" "}
          {formatTk(product.price)} on delivery.
        </p>
      </div>
      ...
      <div className="rounded-md bg-amber-50 p-3 text-sm font-semibold text-amber-900">
        Payment method: Cash on Delivery only. No online payment will be
        collected.
      </div>
